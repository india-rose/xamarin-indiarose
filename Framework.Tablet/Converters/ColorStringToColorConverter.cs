     ...
     /// <returns>L'objet SolidColorBrush</returns>
        public object Convert(object value, Type targetType, object parameter, string language)
        {
            ...
            if (color != null && color.StartsWith("#"))
            {
                ...
                return Color.FromArgb(byte.Parse(opacity, NumberStyles.HexNumber), byte.Parse(r, NumberStyles.HexNumber), byte.Parse(v, NumberStyles.HexNumber), byte.Parse(b, NumberStyles.HexNumber));
            }
            return null;
        }
        ...
        {
            var t = value as Color? ?? new Color();
            return t.ToString();
        }
    }
}
